...
#Function for fetching data entered at the login page
def login():
    try:
...
#Function for logging out from the profile
def logout():
    try:
        session["email"] = None
...
#API Route for add address page
@api.route("/add-address", methods=["POST"])
#Function for fetching data from the add address page
def add_address():
    try:
...
#API Route for create order page
@api.route("/create-order", methods=["POST"])
#Function for fetching data from the create order page
def create_order():
    try:
        user_email = session.get("email")
        user_query = f"select * from users where email='{user_email}';"
...
#API Route for help page
@api.route("/submit-help", methods=["POST"])
#Function for fetching data from the help page
def submit_help():
...
#Function for downloading the attachment from the help page
@api.route("/download/<path:filename>")
def download(filename):
    return send_file(os.path.join(UPLOAD_FOLDER, filename), as_attachment=True)

#API Route for search order page
@api.route("/search-order")
#Funtion for fetching data from the search order page
def search_order():
