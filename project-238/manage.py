...
#Dictionaries containing various data
user_json = [
	{
		"name": "John Doe",
...
#Fuction for resetting the database
def recreate_db():
	db.drop_all()
	db.create_all()
	db.session.commit()

#Function for editing the database
def seeder():
...
#Creating a client command named "rsd" - This function will run the recreate_db and seeder functions
@cli.command()
def rsd():
